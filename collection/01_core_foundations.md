...
| **Data freshness** | Live data until it throws — *"consistent or fails."* You always see the current state, or you get an exception. | **COW:** immutable snapshot taken at `iterator()` time; writes made after that are invisible for the rest of the loop. Guaranteed stale, never inconsistent.<br>**Weakly consistent:** every element present at iterator creation and not since removed is returned **exactly once**, even if the map resizes mid-traversal. Concurrent inserts *may or may not* show up — undefined, so never write logic that depends on seeing them. |
...
| **Risks** | Unexpected CME reaching production.<br>Silent element skips: `hasNext()` doesn't check `modCount`, so if a removal makes `cursor == size` the loop exits early with elements unvisited and no exception. | **COW:** GC pressure and memory spikes on large or write-heavy lists; readers act on stale data. Use only when reads vastly outnumber writes.<br>**CHM:** `size()`, `isEmpty()`, `containsValue()` are estimates under concurrent mutation — never branch on them. `get()`-then-`put()` is not atomic; use `computeIfAbsent`, `merge`, or `putIfAbsent`. |
